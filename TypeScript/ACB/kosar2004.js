...
var kosarOBJ = ObjTombFeltolto(eredmenyek);
//3. Feladat:
function HanyszorSzerepelKosarlabdaTombben(eredmenyekOBJ, miben, mi) {
    var db = 0;
    for (var i = 0; i < eredmenyekOBJ.length; i++) {
        if (eredmenyekOBJ[i][miben] === mi)
            db++;
    }
    return db;
}
...
//4. Feladat:
function DontetlenVanEKosarlabdaTombben(eredmenyekOBJ) {
    for (var i = 0; i < eredmenyekOBJ.length; i++) {
        if (eredmenyekOBJ[i].hazai_pont == eredmenyekOBJ[i].idegen_pont)
            return true;
    }
    return false;
}
...
//5.feladat:
function GrepCsapatnevKosar(eredmenyekOBJ, pattern) {
    var Regex = new RegExp(pattern, 'gi');
    for (var i = 0; i < eredmenyekOBJ.length; i++) {
        if (eredmenyekOBJ[i].hazai.match(Regex))
            return eredmenyekOBJ[i].hazai;
        if (eredmenyekOBJ[i].idegen.match(Regex))
            return eredmenyekOBJ[i].idegen;
    }
    return "";
}
console.log("5. Feladat: barcelonai csapat neve: " + GrepCsapatnevKosar(kosarOBJ, "barcelona"));
//6.feladat:
function MerkozeseketKeresDatumSzerint(eredmenyekOBJ, datum) {
    var retOBJ = [];
    var Regex = new RegExp('^' + datum, 'g');
    for (var i = 0; i < eredmenyekOBJ.length; i++) {
        if (eredmenyekOBJ[i].idopont.match(Regex))
            retOBJ.push(eredmenyekOBJ[i]);
    }
    return retOBJ;
}
function DatumSzerintKiir(eredmenyekOBJ) {
    for (var i = 0; i < eredmenyekOBJ.length; i++) {
...
//7.feladat:
function SzerepelE(t, stadionNeve) {
    for (var i = 0; i < t.length; i++) {
        if (t[i] == stadionNeve)
            return true;
    }
...
function StadionokFeltolt(eredmenyekOBJ) {
    var retStadionok = [];
    for (var i = 0; i < eredmenyekOBJ.length; i++) {
        if (!SzerepelE(retStadionok, eredmenyekOBJ[i].helyszin))
            retStadionok.push(eredmenyekOBJ[i].helyszin);
    }
    return retStadionok;
}
function StadionSzamol(eredmenyekOBJ, stadionok) {
    var retDb = [];
    for (var i = 0; i < stadionok.length; i++) {
        retDb[i] = 0;
    }
    for (var i = 0; i < stadionok.length; i++) {
        for (var j = 0; j < eredmenyekOBJ.length; j++) {
...
function TobbMint20Kiir(stadionok, szereplesek) {
    for (var i = 0; i < szereplesek.length; i++) {
        if (szereplesek[i] > 20) {
            console.log("   " + stadionok[i] + ": " + szereplesek[i]);
        }
    }
}
var stadionok = StadionokFeltolt(kosarOBJ);
var szereplesek = StadionSzamol(kosarOBJ, stadionok);
