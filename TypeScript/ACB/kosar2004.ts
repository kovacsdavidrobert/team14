...
function ObjTombFeltolto(t:string[]):kosar[]{
    let kosarTomb:kosar[] = [];
    for(let i = 0; i < t.length; i++){
        let sor:string[] = t[i].split(';');
        kosarTomb[i] = {
            hazai:      sor[0],
            idegen:     sor[1],
            hazai_pont: Number(sor[2]),
            idegen_pont: Number(sor[3]),
            helyszin: sor[4],
            idopont: sor[5]
        }
    }
...
const kosarOBJ:kosar[] = ObjTombFeltolto(eredmenyek);

//3. Feladat:
function HanyszorSzerepelKosarlabdaTombben(eredmenyekOBJ:kosar[], miben:string, mi:any):number{
    var db:number = 0;
    for(let i = 0; i < eredmenyekOBJ.length ;i++){
        if(eredmenyekOBJ[i][miben] === mi) db++;
    }
    return db;
}

console.log(`3. feladat: Real Madrid: Hazai: ${HanyszorSzerepelKosarlabdaTombben(kosarOBJ, "hazai", "Real Madrid")}, Idegen: ${HanyszorSzerepelKosarlabdaTombben(kosarOBJ, "idegen", "Real Madrid")}.`);


//4. Feladat:
function DontetlenVanEKosarlabdaTombben(eredmenyekOBJ:kosar[]):boolean{
    for(let i = 0; i < eredmenyekOBJ.length ;i++){
        if(eredmenyekOBJ[i].hazai_pont == eredmenyekOBJ[i].idegen_pont) return true;
    }
    return false;
}

console.log(`4. Feladat: Volt döntetlen? ${DontetlenVanEKosarlabdaTombben(kosarOBJ) ? "igen" : "nem"}`);


//5.feladat:
function GrepCsapatnevKosar(eredmenyekOBJ:kosar[], pattern:string):string{
    const Regex:RegExp = new RegExp(pattern, 'gi');
    for(let i = 0; i < eredmenyekOBJ.length ;i++){
        if(eredmenyekOBJ[i].hazai.match(Regex)) return eredmenyekOBJ[i].hazai;
        if(eredmenyekOBJ[i].idegen.match(Regex)) return eredmenyekOBJ[i].idegen;
    }
    return "";
}

console.log(`5. Feladat: barcelonai csapat neve: ${GrepCsapatnevKosar(kosarOBJ, "barcelona")}`);


//6.feladat:
function MerkozeseketKeresDatumSzerint(eredmenyekOBJ:kosar[], datum:string):kosar[]{
    var retOBJ:kosar[] = [];
    const Regex:RegExp = new RegExp('^' + datum, 'g');
    for(let i = 0; i < eredmenyekOBJ.length ;i++){
        if(eredmenyekOBJ[i].idopont.match(Regex)) retOBJ.push(eredmenyekOBJ[i]);
    }
    return retOBJ;
}

function DatumSzerintKiir(eredmenyekOBJ:kosar[]):void{
    for(let i = 0; i < eredmenyekOBJ.length ;i++){
        console.log(`   ${eredmenyekOBJ[i].hazai} - ${eredmenyekOBJ[i].idegen} (${eredmenyekOBJ[i].hazai_pont}:${eredmenyekOBJ[i].idegen_pont})`);
    }
}

DatumSzerintKiir(MerkozeseketKeresDatumSzerint(kosarOBJ, "2004-11-21"));


//7.feladat:

function SzerepelE(t:string[], stadionNeve:string):boolean{
    for(let i = 0;i < t.length;i++){
        if(t[i] == stadionNeve) return true;
    }
    return false;
}

function StadionokFeltolt(eredmenyekOBJ:kosar[]):string[]{
    var retStadionok:string[] = [];
    for(let i = 0; i < eredmenyekOBJ.length ;i++){
        if(!SzerepelE(retStadionok, eredmenyekOBJ[i].helyszin))retStadionok.push(eredmenyekOBJ[i].helyszin);
    }
    return retStadionok;
}

function StadionSzamol(eredmenyekOBJ:kosar[], stadionok:string[]):number[]{
    var retDb:number[] = [];
    for(let i = 0; i < stadionok.length ;i++){
        retDb[i] = 0;
    }
    for(let i = 0; i < stadionok.length ;i++){
        for(let j = 0; j < eredmenyekOBJ.length ;j++){
            if(stadionok[i] == eredmenyekOBJ[j].helyszin){
                retDb[i]++;
            }
        }
    }
...
function TobbMint20Kiir(stadionok:string[], szereplesek:number[]):void{
    for(let i = 0;i < szereplesek.length;i++){
        if(szereplesek[i] > 20){
            console.log(`   ${stadionok[i]}: ${szereplesek[i]}`);
        }
    }
}

let stadionok:string[] = StadionokFeltolt(kosarOBJ);
let szereplesek:number[] = StadionSzamol(kosarOBJ, stadionok);

console.log("7. feladat:");
TobbMint20Kiir(stadionok, szereplesek);
