...
function SportAdatStringTombbolHelsinkiAdatTomb(t) {
    var valaszTomb = [];
...
function PontSzerzesekSzama(Adatok) {
    return Adatok.length;
}
console.log("3. feladat, pontszerző helyezések számának meghatározása:");
console.log("Pontszer\u0151 helyz\u00E9sek sz\u00E1ma: ".concat(PontSzerzesekSzama(HelsinkiAdatok)));
...
function OsszesOlimpiaiPont(Adatok) {
    var osszPont = 0;
    for (var i = 0; i < Adatok.length; i++) {
        if (Adatok[i].helyezes == 1)
            osszPont += 7;
...
function ErmekOsszehasonlitasaKetSportagben(Adatok, sportag1, sportag1Kiir, sportag2, sportag2Kiir) {
    var ermek1 = 0;
    var ermek2 = 0;
    for (var i = 0; i < Adatok.length; i++) {
        if (Adatok[i].sportag == sportag1 && Adatok[i].helyezes <= 3)
            ermek1++;
        if (Adatok[i].sportag == sportag2 && Adatok[i].helyezes <= 3)
            ermek2++;
    }
    if (ermek1 > ermek2)
        return "".concat(sportag1Kiir, " sport\u00E1gban szereztek t\u00F6bb \u00E9rmet.");
    else if (ermek1 < ermek2)
        return "".concat(sportag2Kiir, " sport\u00E1gban szereztek t\u00F6bb \u00E9rmet.");
    else
...
function JavitottKiegeszitettStringTombbotCsinalok(oldHelsinki) {
    var ujHelsinki = [];
    for (var i = 0; i < oldHelsinki.length; i++) {
        ujHelsinki.push(oldHelsinki[i].replace("kajakkenu", "kajak-kenu"));
    }
    for (var i = 0; i < ujHelsinki.length; i++) {
        var olimpiaiPont = 0;
        if (ujHelsinki[i].split(' ')[0] == '1')
            olimpiaiPont = 7;
        if (ujHelsinki[i].split(' ')[0] == '2')
            olimpiaiPont = 5;
        if (ujHelsinki[i].split(' ')[0] == '3')
            olimpiaiPont = 4;
        if (ujHelsinki[i].split(' ')[0] == '4')
            olimpiaiPont = 3;
        if (ujHelsinki[i].split(' ')[0] == '5')
            olimpiaiPont = 2;
        if (ujHelsinki[i].split(' ')[0] == '6')
            olimpiaiPont = 1;
...
function LegtobbSortolo(Adatok) {
    var maxIndex = 0;
    for (var i = 0; i < Adatok.length; i++) {
...
var legnepesebb = LegtobbSortolo(HelsinkiAdatok);
